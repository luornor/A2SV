...
Moreover, they even have their own bank accounts. The state of a bank account is an integer.
...
One of them (the gift of the main ZooVille bank) is the opportunity
to delete the last digit or the digit before last from the state of his bank account no more than once. 
For example, if the state of Ilya's bank account is -123, 
then Ilya can delete the last digit and get his account balance equal to -12, 
also he can remove its digit before last and get the account balance equal to -13. Of course, 
...
if n>=0:
    ...
    positive_n = abs(n)
    first_last_digit_removed =-1*(positive_n//10) # remove the last digit
     
    #remove second to last digit
    second_last_digit_removed = -1*((positive_n // 100) * 10 + positive_n % 10)

    print(max(first_last_digit_removed,second_last_digit_removed))
